...
/**
 * Demo host for the VanityConfigurator module — app/vanity/page.tsx.
 * Shows the module contract in action: catalog + mode in, VanityConfig out.
 * In production the same component mounts inside the contractor door / customer
 * site / portal, and onComplete feeds the BOM/quote.
 */

import { useState } from "react";
import { VanityConfigurator, type VanityConfig } from "@/components/vanity/VanityConfigurator";
import { Check } from "lucide-react";

const money = (n: number) => n.toLocaleString("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 });

export default function VanityDemoPage() {
  const [added, setAdded] = useState<VanityConfig[]>([]);
  ...
  return (
    <div className="min-h-dvh bg-paper">
      <header className="flex items-center gap-3 border-b border-line bg-card px-5 py-4">
        ...
        <div>
          <div className="font-mono text-[10px] uppercase tracking-[0.16em] text-accent">Vanity + Countertop</div>
          <h1 className="font-display text-lg font-bold leading-tight">Configure a vanity</h1>
        </div>
      </header>
