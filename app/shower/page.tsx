...
/**
 * Demo host for the ShowerConfigurator module — app/shower/page.tsx.
 * catalog + mode in, ShowerConfig out. In production this mounts inside the
 * contractor door / customer site and onComplete feeds the BOM/quote.
 */

import { useState } from "react";
import { ShowerConfigurator, type ShowerConfig } from "@/components/shower/ShowerConfigurator";
import { Check } from "lucide-react";

const money = (n: number) => n.toLocaleString("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 });

export default function ShowerDemoPage() {
  const [added, setAdded] = useState<ShowerConfig[]>([]);
  return (
    <div className="min-h-dvh bg-paper">
      <header className="flex items-center gap-3 border-b border-line bg-card px-5 py-4">
        ...
        <div>
          <div className="font-mono text-[10px] uppercase tracking-[0.16em] text-accent">Shower + Tub</div>
          <h1 className="font-display text-lg font-bold leading-tight">Configure a shower</h1>
        </div>
      </header>

      <div className="mx-auto max-w-6xl p-5">
        <ShowerConfigurator mode="dealer" onComplete={(cfg) => setAdded((prev) => [...prev, cfg])} />
